...
static zend_always_inline uint8x16_t neon_is_lu(uint8x16_t x,
...
static zend_always_inline uint8x16_t neon_to_lu(uint8x16_t x,
	...
	for (; i < 4; i++) {
		...
		/* Mask out the higher bits that are used to index in the
		   ...
		   otherwise the returned value is 0. The value in the LUT is
		   the shift amount to convert low->up or up->low case. */
		idx = veorq_u8(x, vdupq_n_u8(i << 6));
		s = vqtbl4q_u8(*((uint8x16x4_t *)shift_LUT[i]), idx);
		shifts = vaddq_u8(shifts, s);
	}
			       ...
			       uint8_t has_shift[4])
{
	...
	for (; len - j > 15; j += 16) {
		...
		uint8x16_t transformed = neon_is_lu(a, shift_LUT, has_shift);
		...
		if (qw.dw[0] != 0 || qw.dw[1] != 0) {
			...
			uint8x16_t b = neon_to_lu(a, shift_LUT, has_shift);
			...
			for (; len - j > 15; j += 16) {
				a = *((uint8x16_t *)(in + j));
				b = neon_to_lu(a, shift_LUT, has_shift);
				vst1q_u8((uint8_t *)(out + j), b);
			}
	...
	if (j < len) {
		...
		ZEND_ASSERT(left_bytes < 16);
		memcpy_15((uint8_t *)&a, in + j, left_bytes);
		transformed = neon_is_lu(a, shift_LUT, has_shift);
